...
def main():
    ...
    X_train, y_train, sc, sc_label = train_data()
    ...
    origin_data = sc_label.inverse_transform(predicted_stock_price)
    ...
    # plt.show()
    process_time = time.strftime("%Y%02m%02d_%02H%02M%02S", time.localtime())
    plt.savefig('pic/SH_%s_ts%d_n%d_%s.png' % (data_utils.get_tag_date2(divide_date), time_step, pre_day, process_time))
